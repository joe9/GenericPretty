...
instance (GPretty f, Constructor c) =>
  ...
  gpretty c@(M1 a) =
     let component = gpretty a
     in if null component
           then [(string . cs . conName) c]
           else [(string . cs . conName) c <+> cat component]
...
instance (GPretty a, GPretty b) =>
         GPretty (a :*: b) where
  gpretty x = [(encloseSep lbrace rbrace comma . prettyArgument) x]

-- prettyArgument xs (x :*: y) =
--   prettyArgument (xs ++ [gpretty x]) y
-- prettyArgument xs x = xs ++ [gpretty x]
-- prettyArgument :: (GPretty a, GPretty b) => (a :*: b) p -> [[Doc]]
-- prettyArgument :: (GPretty a, GPretty b) => (a :*: b) p -> [Doc]
-- prettyArgument :: (Generic a, GPretty (Rep a)) => a -> [Doc]
-- prettyArgument ((M1 x) :*: y) =
--   let component = gpretty x
--   in if null component
--         then prettyArgument y
--         else maybe (prettyArgument y) (\d -> d : prettyArgument y) (headMay component)
-- prettyArgument x = gpretty x
prettyArgument (x :*: y) = xs ++ ys
  where xs = gpretty x
        ys = gpretty y
