...
class Pretty a where
  ...
  default docPrec :: (Generic a ,GPretty (Rep a)) => Int -> a -> Doc
  ...
  doc :: a -> Doc
  default doc :: (Generic a ,GPretty (Rep a)) => a -> Doc
...
instance (GPretty f, Datatype c) => GPretty (M1 D c f) where
  out1 (M1 a) = out1 a
...
instance (Pretty f) => GPretty (K1 t f) where
...
fullPP :: (Pretty a) => (TextDetails -> b -> b)  -- ^Function that handles the text conversion /(eg: 'outputIO')/
...
--
-- > fullPP outputStr ""
prettyStyle :: (Pretty a) => Style -> a -> String
...
prettyLen :: (Pretty a) => Int -> a -> String
